...
class StrategyRepository:
    ...
    def get_by_id(self, strategy_id: str) -> Optional[Strategy]:
        res = self.db.table(self.table_name).select("*").eq("id", strategy_id).single().execute()
        if res.data:
    ...
    def get_one_active(self) -> Optional[Strategy]:
        res = self.db.table(self.table_name).select("*").eq("status", "ACTIVE").limit(1).execute()
        return Strategy.model_validate(res.data[0]) if res.data else None

    def list_by_status(self, status: str) -> List[Strategy]:
        res = self.db.table(self.table_name).select("*").eq("status", status).execute()
