...
@pytest.mark.asyncio
async def test_pipeline_calls_evaluate_all():
    mock_db = MagicMock()
    ...
    with patch("app.core.run_pipeline.fetch_ohlcv", return_value=make_ohlcv()), \
         ...
         patch("app.core.run_pipeline.compute_score", return_value=0.75):
        from app.core.run_pipeline import run_pipeline
        await run_pipeline(pairs=["BTC/USDT"], timeframes=["1h"], ai_eval=True)

    mock_evaluator.evaluate_all.assert_called_once()


@pytest.mark.asyncio
async def test_pipeline_demotes_strategy_on_demote_verdict():
    mock_db = MagicMock()
    mock_db.table.return_value.upsert.return_value.execute.return_value.data = []
    mock_db.table.return_value.update.return_value.eq.return_value.execute.return_value.data = []
...
@pytest.mark.asyncio
async def test_pipeline_ai_errors_do_not_block():
    mock_db = MagicMock()
    mock_db.table.return_value.upsert.return_value.execute.return_value.data = []
...
@pytest.mark.asyncio
async def test_pipeline_without_ai_eval_skips_evaluator():
    mock_db = MagicMock()
    mock_db.table.return_value.upsert.return_value.execute.return_value.data = []
    ...
    with patch("app.core.run_pipeline.fetch_ohlcv", return_value=make_ohlcv()), \
         ...
         patch("app.core.run_pipeline.build_evaluator", return_value=mock_evaluator):
        from app.core.run_pipeline import run_pipeline
        await run_pipeline(pairs=["BTC/USDT"], timeframes=["1h"], ai_eval=False)
